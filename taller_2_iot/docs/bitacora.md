...
## [2026-09-11 12:50] Addendum (Lab 3): trazabilidad del servicio de correo desde Python
- **Qué se hizo:** al iniciar el Laboratorio 3 se auditó el estado del servicio de correo: (a) verificación en el repo local con grep sobre todos los `*.py` (patrones `smtplib`, `SMTP(`, `SMTPServer`, `MailServer`, `sendmail`, `sendgrid`, `mailgun`, `email`, `correo`) → **0 coincidencias de código de envío de correo**; el repo conserva la versión del script sin servicio SMTP propio (las 6 coincidencias de `email|correo` son solo menciones textuales: comentarios de la Rule y mensajes del endpoint). (b) El servicio de mail desde Python que se agregó después del cierre del Lab 2 quedó implantado únicamente en la **copia desplegada en la VM** (`~/taller_iot/`), no versionado en este repo local. (c) Hoy 2026-09-11 12:44 la VM está apagada (`Test-NetConnection` TCP/22=False) → el contenido exacto del servicio desplegado se verificará y documentará aquí cuando la VM vuelva a estar encendida (**acción pendiente anotada**).
- **Por qué:** cerrar el hueco de trazabilidad entre el cierre del Lab 2 (última entrada 2026-09-03) y la adición del servicio, sin inventar detalles no verificables ahora. Se añade al final sin reescribir entradas previas (precedente de entradas complementarias: 2026-09-03 22:40).
- **Resultado / evidencia:** la ruta de alertas por correo vía Rule nativa de Azure IoT Central (`Alerta Temperatura alta`: Temperature > 27 → Email, entrada 2026-09-02 21:18) sigue documentada e **intacta** (no se modificó). Disparador local `/api/llamar-asesor` verificado presente en `python-vm-01.py`, líneas 281–290. **Pendiente:** volcar el script desplegado en la VM a este repo cuando la VM arranque (sin tocar la Rule y sin exponer credenciales).

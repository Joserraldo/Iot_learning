...
//
// UNAB-Ambiental — Laboratorio 2 (Wokwi ESP32)
// Adaptado del ejemplo oficial "Azure_IoT_Central_ESP32" (Azure SDK for C v1.1.8):
//   - DHT22 en GPIO15 (Temperature + Humidity)
//   - Potenciometro en A0/GPIO36 (Iluminance 100..800 lux)
//   - LED rojo en GPIO2 (comando setAlertLed)
// Pines definidos en iot_configs.h.
...
static bool azure_initial_connect = false;
...
/* --- Crypto wrappers (HMAC-SHA256 + Base64) --- */
...
/* --- Callbacks (delegados al PnP Template) --- */
...
void setup()
{
  // En Wokwi el TLS es lento por el cap de CPU del simulador y el IDLE task
  // no resetea el watchdog a tiempo -> timeout largo y sin panic para evitar reboot.
  esp_task_wdt_delete(NULL);
  esp_task_wdt_config_t wdt_cfg;
  wdt_cfg.timeout_ms = 60000;
...
static esp_err_t esp_mqtt_event_handler(esp_mqtt_event_handle_t event)
{
  ...
  switch (event->event_id)
  {
    case MQTT_EVENT_ERROR:
      ...
      {
        LogError(
            "esp_tls_stack_err=%d; esp_tls_cert_verify_flags=%d; connect_return_code=%d",
